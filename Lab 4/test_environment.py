...
class TestEnvironment:
    ...
    def test_show_environment(self, small_env, capsys):
        ...
        assert "Co-ordinates of human:" in captured.out
    ...
    def test_human_generation(self, default_env):
        humans = default_env.humans
        assert len(humans) > 0 
        for x, y in humans:
            assert default_env.grid[x][y] == 2
            assert (x, y) not in [coord for area in default_env.non_accessible for coord in area]
            assert (x, y) not in default_env.bio_hazard
            assert default_env.is_accessible(x, y)