...
def main():
    ...
    for i in range(total_runs):
        path_to_map = r"D:\3-2\Artificial Intelligence\Lab\Lab 8\random_configuration\probability_map.npy"
        print(f"\n>>> Run {i+1} is starting...")
        print("Probability map after this run:")
        env = Environment()
        ...
        robot.policy.update_probability_map(env) #newline added
        data = np.load(path_to_map) # newly added line
        print(data)# newly added line
        perf = Performance(robot)
        perf.calculate()

        folder = Path(r"D:\3-2\Artificial Intelligence\Lab\Lab 8\output_environments_new_20_run")
