...
class Policy:
    def __init__(self):
        try:
            self.p_map = np.load(r"D:\3-2\Artificial Intelligence\Lab\Lab 8\random configuration\probability_map.npy")
            self.threshold = 0.3
        except:
            self.p_map = None
            self.threshold = 0.3
...
#newly used function
    def update_probability_map(self, environment):
        if self.p_map is None:
            return

        for x in range(self.p_map.shape[0]):
            for y in range(self.p_map.shape[1]):
                if self.p_map[x, y] >= self.threshold:
                    if environment.has_bio_hazard(x, y):
                        self.p_map[x, y] += 0.005
                    else:
                        self.p_map[x, y] -= 0.005
                
                self.p_map[x, y] = np.clip(self.p_map[x, y], 0.0, 1.0)
        
        np.save(r"D:\3-2\Artificial Intelligence\Lab\Lab 8\random_configuration\probability_map.npy", self.p_map)
